defmodule LogServer.Router do
  ...
  @hour 60 * 60
  @time_range_thresold 60 * 60 * 24 * 10 # 10day
  get "/query" do
    ...
    %Query{time_range: {begin_query, end_query}} = query_struct =
      cond do
        params["begin"] && params["end"] ->
          {begin_query, _} = Integer.parse(params["begin"])
          {end_query, _} = Integer.parse(params["end"])
          %{query_struct | time_range: {begin_query, end_query}}
        begin_query = params["begin"] ->
          {begin_query, _} = Integer.parse(begin_query)
          end_query = begin_query + @hour
          %{query_struct | time_range: {begin_query, end_query}}
        end_query = params["end"] ->
          {end_query, _} = Integer.parse(end_query)
    ...
    result =
      if end_query - begin_query > @time_range_thresold,
        do: "query time is out of range (for now maximum is 10 day)",
  ...
  patch "/client" do
    IO.inspect(conn, label: "conn client")
    params = conn.params
